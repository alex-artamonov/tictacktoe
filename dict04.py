...
field = {(0, 0): CROSS, (0, 1): EMPTY, (0, 2): EMPTY,
		 (1, 0): EMPTY, (1, 1): EMPTY, (1, 2): CROSS,
...
def attack():
	...
	for dim in vw:
		...
		if values.count(CROSS) == 1 and values.count(EMPTY) == 2:
			# print(dim, keys[values.index(EMPTY)]) 
			keys_lst.append(set(keys))
			# [print(elem) for elem in combinations(keys, 2)]
	# print(keys_lst)
	# [print(mv) for elem in combinations(keys_lst, 2) if (mv := field[elem[0] & elem[1]]) == EMPTY]
	lst_mvs = []
	for elem in combinations(keys_lst, 2):
...
#iter_obj = combinations('ABCD', 2)
#list(print(elem) for elem in iter_obj)
