...
class Gameplay:
    ...
    def play_again_or_leave(self, display=print):
        ...
        if res:
            ...
            # initialize_game()
            self.clear_field()
            self._players_moves[HUMAN_MARK], self._players_moves[COMPUTER_MARK] = self._players_moves[COMPUTER_MARK], self._players_moves[HUMAN_MARK]
        ...
        if len(self.get_legal_moves()) == 0:  # если ходов больше нет
            ...
            while True:
                ...
                if mv in self.legal_moves_str():
                    ...
                    ui.display_field(self.field, 10, self.get_moves_count(), self.current_player, mv)
                    break
                else:
    ...
    def crosses_strategy(self, display=print):
        ...
        free_corners = list(set(self.get_legal_moves()) & set(self.CORNERS))   # находим свободные углы
        ...
        if self.get_moves_count() == 0:   # первый ход - в центр
            ...
            mv = self.check_danger(CROSS, RULE_1) or self.check_danger(NOUGHT, RULE_2) or \
                self.get_fartherst_corner(free_corners, self.current_move) or self.random_move()
        return mv
    ...
    def noughts_strategy(self, display=print):
        ...
        if self.get_moves_count() == 1 and self.current_move in self.SIDES:
            ...
            mv = self.check_danger(NOUGHT, RULE_1) or self.check_danger(CROSS, RULE_2) or self.try_best_moves() or self.random_move()
    ...
    def is_win(self, mark, display=print):
        ...
        for dim in self.DIMENSIONS:
            ...
            if res:
                display(f'Победу одержал: {self.current_player} ходом {self.current_move}')
                return True
    ...
    def greeting(self, display=print):
        ...
        display(self._players_moves[HUMAN_NAME] )
        display(f"Привет, {self._players_moves[HUMAN_NAME]}!")
        self.create_score(self._players_moves[HUMAN_NAME])
        ui = cli.Cli(self._players_moves[COMPUTER_NAME], player_name, self.score)    
        ui.display_score()
