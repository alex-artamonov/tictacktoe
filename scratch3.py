...
def move_player():
    ...
    players_moves[CURRENT_PLAYER] = players_moves[HUMAN_NAME]
    print('from move_player:', players_moves)
    if no_more_moves():
        action_draw()
    else:
        # current_player = human_name
        print(f"Ваш ход '{human_mark}' (первая цифра ряд, вторая - столбец):")
        while True:
            mv = input(INP_INVITE)
            if mv in legal_moves_str():
                ...
                print("mv:", mv)
                print('hi from move_player:else, players_moves:', players_moves)
...
def attack():
    ...
    for dim in DIMENSIONS:
        ...
        lst = [field[cell[0]][cell[1]] for cell in dim]
        if lst.count(EMPTY) == 2 and lst.count(CROSS) == 1:
            # print(lst, dim, lst.index(EMPTY))
            mv = dim[lst.index(EMPTY)]
            print("*Attacking!*", mv)
            # print(mv)
            return mv
        else:
        ...
        # [print(cell) for cell in dim]
        # print('hi from attack for, lst:', lst, dim)
        # if lst.count(EMPTY) == 2 and lst.count(mark) == 1:
        #     print('Hi from attack for if:', dim)
        #     return cell
        # else:
        #     print('Hi from attack else: found nothing')
        #     return None
        # print("from attack for:", dim)
